...
def create_mask_network_sparse_equal(B, num_test_pairs, num_train_pairs, num_valid_pairs):
    ...
    num_valid_pairs_edge = int(num_valid_pairs / 2)
    edge_pairs = num_test_pairs_edge + num_train_pairs_edge + num_valid_pairs_edge
    random_idx = np.random.randint(1, B.nnz, 4 * edge_pairs)
    ...
    idx_train_edges = idx[:, num_test_pairs_edge:num_test_pairs_edge + num_train_pairs_edge]
    idx_valid_edges = idx[:, num_test_pairs_edge + num_train_pairs_edge:edge_pairs]
    ...
    num_valid_pairs_non_edge = int(num_valid_pairs / 2)
    non_edge_pairs = num_test_pairs_non_edge + num_train_pairs_non_edge + num_valid_pairs_non_edge
    ...
    idx_train_non_edges = idx_non_edges[:, num_test_pairs_non_edge:num_test_pairs_non_edge + num_train_pairs_non_edge]
    idx_valid_non_edges = idx_non_edges[:, num_test_pairs_non_edge + num_train_pairs_non_edge:non_edge_pairs]
    idx_test = np.hstack((idx_test_edges, idx_test_non_edges))
    idx_valid = np.hstack((idx_valid_edges, idx_valid_non_edges))
    idx_train = np.hstack((idx_train_edges, idx_train_non_edges))



    return [idx_test, idx_valid, idx_train]
