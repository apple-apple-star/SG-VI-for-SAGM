...
def main(_):
    ...
    seed = args.seed
    step_size_a_w = args.lrw
    step_size_a_pi = args.lrpi

    outfile = outfile + network + 'Seed' + str(seed) + 'L' + str(L) + 'M' + str(M) + 'K' + str(k) + 'burn_in' + str(
        num_burn_in) + 'samples' + str(num_samples) + 'test_ratio' + str(Test_ratio) + 'lrw' + str(step_size_a_w) \
              + 'lrpi' + str(step_size_a_pi) + '.npz'
    print(outfile)
    ...
    with tf.Session(config=config) as sess:
        ...
        for iteration in range(num_burn_in):
            ...
            if (iteration+1) % num_of_samples_in_records == 0:
                ...
                # print("test 1 : " + str(result_test[2]))
                # print("test 2 : " + str(result_test[3]))
                # print("test 3 : " + str(result_test[4]))
                # print("test 4: " + str(result_test[5]))
                ...
                # print("train 1 : " + str(result_train[2]))
                # print("train 2 : " + str(result_train[3]))
                # print("train 3 : " + str(result_train[4]))
                # print("train 4: " + str(result_train[5]))
    ...
    np.savez(outfile, AvgAUC=avg_auc_test, TestAUCvector=TestAUCvector, TestPPvector=TestPPvector,
             TrainAUCvector=TrainAUCvector, TrainPPvector=TrainPPvector, TimeVector=TimeVector, w=w)
