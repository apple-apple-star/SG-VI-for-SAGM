...
def run_one_random_GRVI(B, args, param_set, test_pairs, train_pairs, model, Train):
    ...
    outfile_name = outfile + network + 'lr_lambda' + str(step_size_a_lambda) + 'lr_phi' + str(step_size_a_phi) + \
                   'lr_tau' + str(step_size_a_tau) + 'riemann' + str(riemann) + 'elbo' + str(elbo)\
                   + 'mc' + str(mc) + 'L' + str(L) + 'M' + str(M) + 'K' + str(k) + 'max iterations' \
                   + str(num_max_iterations) + 'test_ratio' + str(Test_ratio) + '.npz'
    ...
    full_b[test_pairs[0, :], test_pairs[1, :]] = 0
    full_b[test_pairs[1, :], test_pairs[0, :]] = 0
    ...
    sCur_phi_bar_holder = tf.placeholder(tf.float64)
    sCur_lambda_bar_holder = tf.placeholder(tf.float64)
    sCur_tau_holder = tf.placeholder(tf.float64)
    ...
    with tf.Session(config=config) as sess:
        ...
        sess.run(init, feed_dict={sCur_phi_bar_holder: model.sCur_phi_bar, sCur_lambda_bar_holder: model.sCur_lambda_bar,
                                  sCur_tau_holder: model.sCur_tau, tau_holder: model.tau,
                                  lambda_bar_holder: model.lambda_bar, phi_bar_holder: model.phi_bar})
        ...
        np.savez(outfile_name, TestAUCvector=TestAUCvector, TestPPvector=TestPPvector, TrainAUCvector=TrainAUCvector,
                 TrainPPvector=TrainPPvector, TimeVector=TimeVector, TestELBOvector=TestELBOvector,
                 TrainELBOvector=TrainELBOvector, w=w)
